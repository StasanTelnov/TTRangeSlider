...
- (void)initialiseControl {
    ...
    _handleShadowColor = self.tintColor;
    _handleShadowRadius = 1.0;
    _handleShadowOffset = CGSizeMake(0.0, 0.0);
    ...
    self.rightHandle.shadowColor = self.handleShadowColor.CGColor;
...
-(void)setHandleShadowColor:(UIColor *)handleShadowColor {
    ...
    [self setDefaultShadowOpacity];
}

-(void)setHandleShadowRadius:(CGFloat)handleShadowRadius {
    _handleShadowRadius = handleShadowRadius;
    self.leftHandle.shadowRadius = handleShadowRadius;
    self.rightHandle.shadowRadius = handleShadowRadius;
    
    [self setDefaultShadowOpacity];
}

-(void)setHandleShadowOffset:(CGSize)handleShadowOffset {
    _handleShadowOffset = handleShadowOffset;
    self.leftHandle.shadowOffset = handleShadowOffset;
    self.rightHandle.shadowOffset = handleShadowOffset;
    
    [self setDefaultShadowOpacity];
}

-(void)setHandleShadowOpacity:(float)handleShadowOpacity {
    _handleShadowOpacity = handleShadowOpacity;
    self.leftHandle.shadowOpacity = handleShadowOpacity;
    self.rightHandle.shadowOpacity = handleShadowOpacity;
    
}

-(void)setHandleShadowPath:(CGPathRef)handleShadowPath {
    _handleShadowPath = handleShadowPath;
    self.leftHandle.shadowPath = handleShadowPath;
    self.rightHandle.shadowPath = handleShadowPath;
...
#pragma mark - set default shadow opacity
-(void)setDefaultShadowOpacity {
    if (!self.leftHandle.shadowOpacity) {
        self.leftHandle.shadowOpacity = 1.0;
    }
    if (!self.rightHandle.shadowOpacity) {
